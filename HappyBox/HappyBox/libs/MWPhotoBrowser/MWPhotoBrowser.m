...
// Private
@interface MWPhotoBrowser () <PhotorollNavBarDelegate, PhotorollToolbarDelegate> {
    ...
    BOOL _navBarHidden;
    PhotorollNavBar *_customNavigationBar;
    PhotorollToolbar *_customToolbar;
...
- (void)viewDidLoad {
    ...
    if (_customNavigationBar) {
        _customNavigationBar.frame = CGRectMake(0,
                                                0,
                                                IDEVICE_WIDTH,
                                                ViewHeight(_customNavigationBar));
        [self.view addSubview:_customNavigationBar];
    }
    
    if (_customToolbar) {
        _customToolbar.frame = CGRectMake(0,
                                          IDEVICE_HEIGHT - ViewHeight(_customToolbar),
                                          IDEVICE_WIDTH,
                                          ViewWidth(_customToolbar));
...
- (void)updateNavigation {
	...
	if ([self numberOfPhotos] > 1) {
        NSString *title = [NSString stringWithFormat:@"%i %@ %i", _currentPageIndex+1, NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), [self numberOfPhotos]];
		self.title = title;
        _customNavigationBar.title = title;
	} else {
		self.title = nil;
        _customNavigationBar.title = nil;
	}
...
- (void)setBackgroungImage:(UIImage *)backgroundImage
{
    _backgroundImage = backgroundImage;
}

- (void)setBackgroungColor:(UIColor *)backgroundColor
{
    _backgroundColor = backgroundColor;
}

- (void)setCustomNavigationBar:(UIView *)customNavBar
{
    _customNavigationBar = (PhotorollNavBar *)customNavBar;
...
- (void)setCustomToolBar:(UIView *)customToolBar
{
    _customToolbar = (PhotorollToolbar *)customToolBar;
    _customToolbar.delegate = self;
}


#pragma mark - @protocol PhotorollNavBarDelegate <NSObject>
...
#pragma mark - @protocol PhotorollToolbarDelegate <NSObject>
...
{
    [self.delegate photoBrowser:self photoWasDeletedAtIndex:_currentPageIndex];
}
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    InstagramActivity *ia = [InstagramActivity new];
                    
                    self.activityViewController = [[UIActivityViewController alloc] initWithActivityItems:items applicationActivities:@[ia]];
//                    self.activityViewController.excludedActivityTypes = @[UIActivityTypeMessage, UIActivityTypePrint, UIActivityTypeCopyToPasteboard, UIActivityTypeAssignToContact, UIActivityTypeSaveToCameraRoll];
                    ...
                    [self.activityViewController setCompletionHandler:^(NSString *activityType, BOOL completed) {
                        if ([activityType isEqualToString:ia.activityType]) {
                            [weakSelf saveToInstagram];
                        }
...
{
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    UIImage *shareImage = [photo underlyingImage];
    NSString *instagramShareFilePath = [NSString stringWithFormat:@"%@/PhotomatPhoto.igo", CachesDirectory()];;
    
    [UIImageJPEGRepresentation(shareImage, 1.0) writeToFile:instagramShareFilePath atomically:YES];
    
    NSURL *url = [[NSURL alloc] initFileURLWithPath:instagramShareFilePath];
    
    documentInteractionController = [UIDocumentInteractionController interactionControllerWithURL:url];
    documentInteractionController.UTI = @"com.instagram.exclusivegram";
    NSString *shareText = NSLocalizedString(@"key.instagramm_share_text_pro", nil);
    documentInteractionController.annotation = [NSDictionary dictionaryWithObject:shareText forKey:@"InstagramCaption"];
    [documentInteractionController presentOpenInMenuFromRect:_customToolbar.frame inView:self.view animated:YES];
}
